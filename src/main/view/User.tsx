...
export default class User extends React.Component<{content:any},{browse:boolean, data:any}> {
  ...
  confirmAdding(selected){
    ...
    FetchWithAuth(process.env.REACT_APP_REMOTE_URL + '/access/assign/'+ids.join(',')+'/'+this.props.content.cid)
        .then(res => res.json())
        .then((data) => {
          this.fetchRoles();
        } );
  ...
  render(){
    ...
    return <div className="block">
          <h3>
            ...
            {this.state.browse&&<Browse config={util.getConfig().browse} multi={true} selected={this.state.data} onConfirm={(selected)=>this.confirmAdding(selected)} onClose={()=>this.setState({browse:false})} trigger={true} contenttype={["role"]} />}
          </h3>
          <ul>
